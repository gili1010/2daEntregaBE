...
class ContenedorMongoDb {
    ...
    async actualizar(nuevoElem) {
        try {
            const personas = await dbPersonas.updateOne({id:1},{$set:nuevoElem}).toArray()
            console.log(id);   
    ...
    async borrar(id) {
        try {
            const personas = await dbPersonas.deleteOne({id: Number(id)}).toArray()
                return personas
            } catch (error) {
                return []
            } 
    ...
    async borrarAll() {
        try {
            const personas = await dbPersonas.deletemany({}).toArray()
                return personas
            } catch (error) {
                return []
            } 
